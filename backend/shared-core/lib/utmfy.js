...
function buildProductEntry({ id, name, price }) {
    return {
        id: pickText(id) || 'gta_vi',
        name: pickText(name) || 'GTA VI',
        planId: null,
        planName: null,
        quantity: 1,
...
function buildUtmfyOrder(eventName, payload = {}, cfg = {}) {
    ...
    const safeEmail = customerEmail || `lead.${safeOrderId}@rockstargames.app`;
    const safeName = customerName || 'Cliente Rockstar';
    ...
    if (products.length === 0) {
        products.push(buildProductEntry({
            ...
            name: upsellEnabled ? upsellTitle : 'GTA VI',
            price: totalAmount
        }));
    }
    ...
    return {
        orderId,
        platform: cfg.platform || 'Rockstar GTA VI',
        paymentMethod: 'pix',
        status,
        createdAt,
        approvedDate,
