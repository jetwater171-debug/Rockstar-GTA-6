...
function isParadisePaidStatus(statusRaw) {
    ...
    return ['approved', 'completed', 'paid', 'authorized'].includes(status);
}
...
function isParadisePendingStatus(statusRaw) {
    const status = normalizeStatus(statusRaw);
    return ['pending', 'processing', 'under_review', 'review', 'in_review', 'waiting_payment', 'awaiting_payment', 'created', 'success'].includes(status);
}
