...
const defaultSettings = {
    ...
    utmfy: {
        ...
        platform: String(process.env.UTMFY_PLATFORM || 'Rockstar GTA VI').trim() || 'Rockstar GTA VI'
    },
    pushcut: {
        enabled: false,
        pixCreatedUrl: '',
...
function normalizeSettingsValue(value = {}) {
    ...
    const valuePixel = source.pixel && typeof source.pixel === 'object'
        ? source.pixel
        : {
            enabled: tracking.browserPixel !== false && Boolean(tracking.metaPixel),
            id: tracking.metaPixel || '',
            backupId: tracking.metaBackupPixel || '',
            capi: {
                enabled: tracking.serverEvents === true,
                accessToken: tracking.metaAccessToken || '',
                backupAccessToken: tracking.metaBackupAccessToken || '',
                testEventCode: tracking.metaTestEventCode || '',
                backupTestEventCode: tracking.metaBackupTestEventCode || ''
            }
        };
    const valueTikTokPixel = source.tiktokPixel && typeof source.tiktokPixel === 'object'
        ? source.tiktokPixel
        : {
            enabled: tracking.browserPixel !== false && Boolean(tracking.tiktokPixel),
            id: tracking.tiktokPixel || ''
        };
    const legacyGateways = source.gateways && typeof source.gateways === 'object' ? source.gateways : null;
    const paymentsInput = source.payments && typeof source.payments === 'object'
        ? source.payments
        : legacyGateways
            ? {
                activeGateway: legacyGateways.activeGateway || legacyGateways.active,
                gatewayOrder: legacyGateways.gatewayOrder,
                gateways: legacyGateways
            }
            : {};
    return {
        ...defaultSettings,
        ...
        payments: buildPaymentsConfig(paymentsInput),
        features: {
