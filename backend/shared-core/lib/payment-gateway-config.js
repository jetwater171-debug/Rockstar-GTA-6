...
function buildBravoPayConfig(raw = {}) {
    const webhookSecret =
        ...
        toText(raw.webhookToken) ||
        toText(process.env.BRAVOPAY_WEBHOOK_SECRET);
    return {
        enabled: raw.enabled === true,
        baseUrl:
            toText(raw.baseUrl) ||
            toText(raw.base_url) ||
            toText(process.env.BRAVOPAY_BASE_URL) ||
            DEFAULT_BRAVOPAY_BASE_URL,
        apiKey:
            toText(raw.apiKey) ||
            toText(raw.api_key) ||
            toText(raw.token) ||
            toText(process.env.BRAVOPAY_API_KEY),
        webhookSecret,
        webhookToken: webhookSecret,
        webhookTokenRequired: raw.webhookTokenRequired !== undefined
            ? !!raw.webhookTokenRequired
            : true,
        timeoutMs: Number(raw.timeoutMs || process.env.BRAVOPAY_TIMEOUT_MS || 12000),
        expiresIn: Number(raw.expiresIn || raw.expires_in || process.env.BRAVOPAY_EXPIRES_IN || 3600),
        description:
            toText(raw.description) ||
            toText(process.env.BRAVOPAY_DESCRIPTION)
    };
}
