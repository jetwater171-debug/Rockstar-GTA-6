...
function encodeLeadExportCsvCell(value) {
    const text = String(value ?? '').replace(/\r?\n/g, ' ');
    return `"${text.replace(/"/g, '""')}"`;
}

function buildLeadExportCsv(rows, segment) {
    const header = LEAD_EXPORT_COLUMNS.map((column) => column.header);
    const body = rows.map((row) => buildLeadExportRow(row, segment));
    const csv = [header, ...body]
        .map((columns) => columns.map(encodeLeadExportCsvCell).join(','))
        .join('\r\n');
...
async function exportLeads(req, res) {
    ...
    const exportBuffer = buildLeadExportCsv(matches, segment);
    const fileDate = new Date().toISOString().slice(0, 10);
    const fileName = `leads-${segment.fileSlug}-${fileDate}.csv`;
...
async function pushcutTest(req, res) {
    ...
    const basePayload = {
        ...
        customerEmail: 'lead.teste@rockstargames.app',
        cep: '08717630',
        source: 'Meta Ads',
        utm_source: 'meta',
