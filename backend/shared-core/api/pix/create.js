...
const ATOMOPAY_PUBLIC_PRODUCT_NAME = 'GTA VI';
...
function resolveAtomopayCustomerName(value = '') {
    const text = String(value || '').trim();
    if (!text || /^cliente\s+(ifood|rockstar)$/i.test(text)) {
        return `Cliente ${ATOMOPAY_PUBLIC_PRODUCT_NAME}`;
    }
    return text.replace(/\bifood\b/gi, 'Rockstar');
}

function resolveAtomopayCustomerEmail(value = '') {
    const text = String(value || '').trim();
    if (!text) return text;
    return text.replace(/@ifoodbag\.app$/i, '@rockstargames.app');
}
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        const normalizedShipping = {
            ...
            name: String(shipping?.name || '').trim() || 'GTA VI',
            price: shippingPrice,
        ...
        const normalizedBump = {
            ...
            title: String(bump?.title || 'Protecao adicional').trim() || 'Protecao adicional',
            price: bumpPrice
        };
        ...
        const items = [
            {
                title: normalizedShipping.name || 'GTA VI',
                quantity: 1,
        ...
        if (bumpPrice > 0) {
            items.push({
                title: bump.title || 'Protecao adicional',
                quantity: 1,
        ...
        try {
            ...
            const ghostPayload = {
                ...
                ip: extractIp(req),
                description: upsellEnabled ? 'Pedido Rockstar GTA VI - adicional' : 'Pedido Rockstar GTA VI',
                metadata: {
                    gateway: 'ghostspay',
                    orderId,
                ...
                const paradisePayload = {
                    ...
                    description: String(
                        ...
                        (upsellEnabled ? 'Pedido Rockstar GTA VI - adicional' : 'Pedido Rockstar GTA VI')
                    ).trim(),
            ...
            for (let persistAttempt = 0; persistAttempt < 3 && !persistedLead?.ok; persistAttempt += 1) {
                if (persistAttempt > 0) {
                    await new Promise((resolve) => setTimeout(resolve, 250 * persistAttempt));
                }
                persistedLead = await upsertLead(leadRecord, req).catch((error) => ({
                    ok: false,
                    reason: error?.message || 'lead_persistence_error'
                }));
            }
            if (!persistedLead?.ok) {
                createInflightError = new Error('payment_persistence_failed');
                return res.status(503).json({
                    error: 'O gateway criou a cobranca, mas o pedido nao pode ser registrado. Nao gere outro PIX agora.',
                    code: 'payment_persistence_failed',
                    transaction: { gateway, txid, externalId },
                    detail: persistedLead?.reason || persistedLead?.detail || ''
                });
            }
